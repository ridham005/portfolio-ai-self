...
export default function HoverMeshText({
  text = "ENGINEERING\nHIGH-PERFORMANCE\nDIGITAL SYSTEMS.",
  color = "#ffffff",
  fontFamily = "Inter",
  fontWeight = 700,
  fontSize = 68,
  colorSplit = true,
  customColors = ["#0066b1", "#e22718"], // BMW M Blue and M Red
  force = 22,
  className = "",
  style = {}
}) {
  ...
  customColorsRef.current = Array.isArray(customColors) ? customColors.map(parseColor) : [[0, 0.4, 0.7], [0.89, 0.15, 0.09]];
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      let cA = cols[0] || [0, 0.4, 0.7];
      let cB = cols[1] || [0.89, 0.15, 0.09];
  ...
  return (
    <div
      ref={wrapperRef}
      className={`relative w-full h-[260px] md:h-[320px] overflow-hidden select-none cursor-crosshair ${className}`}
      style={{ ...style }}
    >
